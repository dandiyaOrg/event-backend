...
// Allowed categories
const ALLOWED_CATEGORIES = ["Group", "Stag Male", "Stag Female", "Couple", "Full Pass"];

// Router: /event/:eventId/subevent/:subEventId/pass -- working this part but what about final price 
const CreateThePass = asyncHandler(async (req, res, next) => {
    try {
        const { category, total_price, discount_percentage, validity } = req.body;

        // Required fields check
        if (!category || !total_price || !validity) {
            return next(new ApiError(400, "Category, total_price, and validity are required"));
        }

        // Validate category
        if (!ALLOWED_CATEGORIES.includes(category)) {
            return next(new ApiError(400, `Category must be one of: ${ALLOWED_CATEGORIES.join(", ")}`));
        }
        ...
        return res.status(201).json(new ApiResponse(true, 201, "Pass created successfully", data));
    } catch (error) {
...
const DeleteThePass = asyncHandler(async (req, res, next) => {
    try {
        ...
        return res.status(200).json(new ApiResponse(true, 200, "Pass deleted successfully"));
    } catch (error) {
        return next(new ApiError(500, error.message || "Internal Server Error"));
    }
});
...
const UpdateThePass = asyncHandler(async (req, res, next) => {
    try {
        const { passId } = req.params;
        const { category, total_price, discount_percentage, validity } = req.body;
        ...
        if (discount_percentage !== undefined) pass.discount_percentage = discount_percentage;
        if (validity !== undefined) pass.validity = validity;

        await pass.save(); 

        return res.status(200).json(new ApiResponse(true, 200, "Pass updated successfully", pass));
    } catch (error) {
        return next(new ApiError(500, error.message || "Internal Server Error"));
    }
});
...
const GetAllThePass = asyncHandler(async (req, res, next) => {
    try {
...
const GetPassById = asyncHandler(async (req, res, next) => {
    try {
        ...
        return res.status(200).json(
            new ApiResponse(true, 200, "Pass retrieved successfully", pass)
        );
    } catch (error) {
        return next(new ApiError(500, error.message || "Internal Server Error"));
    }
...
export { CreateThePass, DeleteThePass ,  UpdateThePass,GetPassById,GetAllThePass};
